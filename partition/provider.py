...
def partition2ply(filename, xyz, components):
    ...
    def random_color(): return random.randint(0, 255)
    color = np.zeros(xyz.shape)
    for i_com in range(0, len(components)):
        color[components[i_com], :] = [
...
def prediction2ply(filename, xyz, prediction, n_label, dataset):
    ...
    for i_label in range(0, n_label + 1):
        color[np.where(prediction == i_label),
              :] = get_color_from_label(i_label, dataset)
    prop = [('x', 'f4'), ('y', 'f4'), ('z', 'f4'),
            ('red', 'u1'), ('green', 'u1'), ('blue', 'u1')]
    vertex_all = np.empty(len(xyz), dtype=prop)
    for i in range(0, 3):
...
def error2ply(filename, xyz, rgb, labels, prediction):
    ...
    for i_ver in range(0, len(labels)):
        ...
        color_hsv = list(colorsys.rgb_to_hsv(
            color_rgb[i_ver, 0], color_rgb[i_ver, 1], color_rgb[i_ver, 2]))
        ...
        color_rgb[i_ver, :] = list(colorsys.hsv_to_rgb(
            color_hsv[0], color_hsv[1], color_hsv[2]))
...
def spg2ply(filename, spg_graph):
    ...
    vertex_val = np.empty(
        (spg_graph['sp_centroids']).shape[0],
        dtype=vertex_prop)
    for i in range(0, 3):
    ...
    ply = PlyData([PlyElement.describe(vertex_val, 'vertex'),
                  PlyElement.describe(edges_val, 'edge')], text=True)
    ply.write(filename)
# ------------------------------------------------------------------------------
...
def read_semantic3d_format(
        data_file, n_class, file_label_path, voxel_width, ver_batch):
        ...
        if voxel_width > 0:
            if has_labels > 0:
                xyz_sub, rgb_sub, labels_sub, objets_sub = libply_c.prune(
                    xyz_full, voxel_width, rgb_full, labels_full, np.zeros(
                        1, dtype='uint8'), n_class, 0)
                labels = np.vstack((labels, labels_sub))
                del labels_full
            else:
                xyz_sub, rgb_sub, l, o = libply_c.prune(
                    xyz_full, voxel_width, rgb_full, np.zeros(
                        1, dtype='uint8'), np.zeros(
                        1, dtype='uint8'), 0, 0)
    ...
    if n_class > 0:
        for (i_chunk, (vertex_chunk, label_chunk)) in \
            ...
            print("processing lines %d to %d" %
                  (i_chunk * ver_batch, (i_chunk + 1) * ver_batch))
            xyz, rgb, labels = process_chunk(
                vertex_chunk, label_chunk, 1, xyz, rgb, labels)
    else:
        for (i_chunk, vertex_chunk) in enumerate(pd.read_csv(
                data_file, delimiter=' ', chunksize=ver_batch, header=None)):
            print("processing lines %d to %d" %
                  (i_chunk * ver_batch, (i_chunk + 1) * ver_batch))
            xyz, rgb, dump = process_chunk(
                vertex_chunk, None, 0, xyz, rgb, None)
    ...
    while True:
        try:
            ...
            if ver_batch > 0:
                ...
                vertices = np.pd.read_csv(
                    data_file, sep=' ', header=None).values
                break
...
def read_ply(filename):
    ...
    try:
        rgb = np.stack([plydata['vertex'][n]
                        for n in ['red', 'green', 'blue']], axis=1).astype(np.uint8)
    except ValueError:
        rgb = np.stack([plydata['vertex'][n]
                        for n in ['r', 'g', 'b']], axis=1).astype(np.float32)
    if np.max(rgb) > 1:
        rgb = rgb
    try:
...
def write_ply_obj(filename, xyz, rgb, labels, object_indices):
    ...
    prop = [('x', 'f4'), ('y', 'f4'), ('z', 'f4'), ('red', 'u1'), ('green',
                                                                   'u1'), ('blue', 'u1'), ('label', 'u1'), ('object_index', 'uint32')]
    vertex_all = np.empty(len(xyz), dtype=prop)
...
def write_spg(file_name, graph_sp, components, in_component):
    ...
    data_file.create_dataset(
        'sp_labels',
        data=graph_sp["sp_labels"],
        dtype='uint32')
    data_file.create_dataset(
        'sp_centroids',
        data=graph_sp["sp_centroids"],
        dtype='float32')
    data_file.create_dataset(
        'sp_length',
        data=graph_sp["sp_length"],
        dtype='float32')
    data_file.create_dataset(
        'sp_surface',
        data=graph_sp["sp_surface"],
        dtype='float32')
    data_file.create_dataset(
        'sp_volume',
        data=graph_sp["sp_volume"],
        dtype='float32')
    data_file.create_dataset(
        'sp_point_count',
        data=graph_sp["sp_point_count"],
        dtype='uint64')
    ...
    data_file.create_dataset(
        'se_delta_mean',
        data=graph_sp["se_delta_mean"],
        dtype='float32')
    data_file.create_dataset(
        'se_delta_std',
        data=graph_sp["se_delta_std"],
        dtype='float32')
    data_file.create_dataset(
        'se_delta_norm',
        data=graph_sp["se_delta_norm"],
        dtype='float32')
    data_file.create_dataset(
        'se_delta_centroid',
        data=graph_sp["se_delta_centroid"],
        dtype='float32')
    data_file.create_dataset(
        'se_length_ratio',
        data=graph_sp["se_length_ratio"],
        dtype='float32')
    data_file.create_dataset(
        'se_surface_ratio',
        data=graph_sp["se_surface_ratio"],
        dtype='float32')
    data_file.create_dataset(
        'se_volume_ratio',
        data=graph_sp["se_volume_ratio"],
        dtype='float32')
    data_file.create_dataset(
        'se_point_count_ratio',
        data=graph_sp["se_point_count_ratio"],
        dtype='float32')
# -----------------------------------------------------------------------------
...
def read_spg(file_name):
    ...
    graph["sp_centroids"] = np.array(
        data_file["sp_centroids"], dtype='float32')
    ...
    graph["sp_point_count"] = np.array(
        data_file["sp_point_count"], dtype='uint64')
    graph["se_delta_mean"] = np.array(
        data_file["se_delta_mean"], dtype='float32')
    graph["se_delta_std"] = np.array(
        data_file["se_delta_std"], dtype='float32')
    graph["se_delta_norm"] = np.array(
        data_file["se_delta_norm"], dtype='float32')
    graph["se_delta_centroid"] = np.array(
        data_file["se_delta_centroid"], dtype='float32')
    graph["se_length_ratio"] = np.array(
        data_file["se_length_ratio"], dtype='float32')
    graph["se_surface_ratio"] = np.array(
        data_file["se_surface_ratio"], dtype='float32')
    graph["se_volume_ratio"] = np.array(
        data_file["se_volume_ratio"], dtype='float32')
    graph["se_point_count_ratio"] = np.array(
        data_file["se_point_count_ratio"], dtype='float32')
        ...
        try:
            if ver_batch > 0:
                ...
                vertices = pd.read_csv(
                    ...
                    sep=' ',
                    nrows=ver_batch,
                    header=None if i_rows is None else i_rows -
                    1).values
            else:
                # vertices = np.genfromtxt(data_file
                #        , delimiter=' ')
                vertices = pd.read_csv(
                    data_file, delimiter=' ', header=None).values
                break
        except (StopIteration, pd.errors.ParserError):
            # end of file
            break
...
def compute_gt_connected_components(
        n_ver, edg_source, edg_target, is_transition, cutoff):
